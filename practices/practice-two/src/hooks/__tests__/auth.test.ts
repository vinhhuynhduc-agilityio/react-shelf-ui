...
describe('useRegisterUser', () => {
  ...
  it('should call registerUser with correct payload and set isSuccess', async () => {
    ...
    (registerUser as jest.Mock).mockResolvedValue({ id: '1', ...payload, avatarUrl: '', registerNumber: '', phoneNumber: '', bio: '', countryCode: '' });
  ...
  it('should handle error correctly', async () => {
    ...
    (registerUser as jest.Mock).mockImplementation(() => Promise.reject(new Error(errorMessage)));
    const payload = {
      fullName: 'Test User',
      email: 'test@email.com',
      password: '123456',
    ...
    const { result } = renderHook(() => useRegisterUser(), { wrapper });
    await expect(result.current.mutateAsync(payload)).rejects.toThrow(errorMessage);
