...
export const TextField = <T extends FieldValues>({
  ...
  const errorClass = clsx(
    "text-red-500 text-xs sm:text-sm md:text-base",
    validErrorMargins.includes(withErrorMargin) ? withErrorMargin : "mt-1"
  );

  const containerClass = clsx("flex", {
    "flex-col gap-[8px] sm:gap-[10px] md:gap-[12px]": vertical,
  ...
  return (
    <div>
      <div className={containerClass}>
        <label
          htmlFor={name}
          className={`${labelClass} text-xs sm:text-sm md:text-base`}
        >
          {label}
        </label>
